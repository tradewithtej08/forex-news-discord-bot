...
async function resolveConfiguredChannel(config) {
  try {
    ...
    if (!isSupportedTextChannel(channel)) {
      ...
      if (fallback) { setGuildChannel(guild.id, fallback.id); return fallback; }
      return null;
    }
    return channel;
  } catch { return null; }
...
async function postDailyToGuild(config, forcePost = false) {
  ...
  for (let i = 0; i < embeds.length; i++) {
    if (i === 0) await channel.send(everyonePayload(embeds[i])); else await channel.send({ embeds: [asEmbed(embeds[i])] });
  }
...
async function process15MinuteReminders(config, events, now) {
  ...
  for (const event of events) {
    ...
    if (due && !wasSent(config.guild_id, event.key, type)) {
      await channel.send(everyonePayload(buildReminderEmbed(event)));
...
async function schedulerTick() {
  ...
  try {
    ...
    if (now.hour === 0 && now.minute === 0) {
      await recoverMissingGuildConfigs(); await refreshNews(true);
...
client.once(Events.ClientReady, async readyClient => {
  console.log(`Logged in as ${readyClient.user.tag}`); console.log(`Connected to ${readyClient.guilds.cache.size} Discord server(s).`);
  await recoverMissingGuildConfigs(); console.log(`Configured ${getEnabledGuilds().length} server(s) for news.`);
  await refreshNews(false).catch(console.error); await schedulerTick(); setInterval(schedulerTick, 30_000);
});
client.on(Events.GuildCreate, async guild => {
  console.log(`Joined guild ${guild.id} (${guild.name}). Connected guilds: ${client.guilds.cache.size}`);
  const channel = findNewsChannel(guild); if (channel && !getGuildConfig(guild.id)) { setGuildChannel(guild.id, channel.id); console.log(`[guild ${guild.id}] Auto-configured #${channel.name}`); }
});
...
client.on(Events.InteractionCreate, async interaction => {
  ...
  try {
    ...
    if (interaction.commandName === "testnews") {
      ...
      if (!cfg) return interaction.editReply("Run `/setup` first."); await postDailyToGuild(cfg, true);
      return interaction.editReply("✅ Test news post sent to the configured channel with @everyone.");
    }
    if (interaction.commandName === "remove") { removeGuild(interaction.guildId); return interaction.reply({ content: "✅ Forex news alerts have been disabled for this server.", ephemeral: true }); }
  } catch (err) {
    console.error("Command error:", err); const msg = "❌ Something went wrong. Check the bot logs.";
    if (interaction.deferred || interaction.replied) await interaction.editReply(msg).catch(() => {}); else await interaction.reply({ content: msg, ephemeral: true }).catch(() => {});
  }
});
